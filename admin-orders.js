...
// and order status transitions (ship / deliver / cancel + stock return).
...
function drawO(){
  ...
  filteredORD.forEach(o=>{
    ...
    // paymentScreenshot is a data: URL from a canvas re-encode (see checkout.html),
    // so it should never contain quotes/markup — but esc() as defense in depth.
...
function printInvoice(id){
  ...
  // Invoice opens in a new same-origin window via document.write, so any
  // unescaped customer text here would run with full access to this app's
  // session (cookies/localStorage) — always escape before interpolating.
  ...
  w.document.write(`
    ...
    <head>
      ...
      <style>
        ...
        .total{font-size:20px; font-weight:bold; color:#ff6600}
