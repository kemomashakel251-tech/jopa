...
function initApp(){
  Promise.all([
  ...
  ]).then(([{ initializeApp }, { getFirestore, collection, addDoc, doc, getDoc, getDocs, setDoc, updateDoc, onSnapshot, serverTimestamp, increment }]) => {
    ...
    db = getFirestore(app);
    window.db = db;
    window.setDoc = setDoc; window.updateDoc = updateDoc; window.doc = doc; window.getDoc = getDoc; window.getDocs = getDocs;
